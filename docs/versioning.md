...
## telegram-bot-api today

**This repository does not use `1.2.3`.** Its versions look like this:

```text
v10.3-1    first build of the Bot API 10.3 server
v10.3-2    same Bot API version, something in this repository changed
v10.4-1    the pin moved to Bot API 10.4
```

The reason is that the server's version is Telegram's, not ours. `EXPECTED_BOT_API`
in the Dockerfile is checked against what the pinned sources declare and again
against what the pushed image reports, so the first half of the tag is a fact,
not a choice. The build number after it is the only part this repository
controls, and it behaves like a patch: base image, entrypoint, labels, or a
different upstream commit within the same Bot API version.

There is no alpha, beta, or rc line here, and that is deliberate. A release does
not build anything — it re-tags a digest the build workflow already pushed and
verified, and re-runs `--version` against it. The proving happens before the
tag, using `deploy/probe.sh` against a server no real bot is on.

The branch rule is the same as everywhere else: tags go on `main`.
...
- the Bot API version the server implements, which is why it is in the tag
- `TELEGRAM_FILES_DIR` and the working-directory split, which decides what a
  bot's mount exposes
- the network alias `telegram-bot-api`, which older bot configs still resolve
- required environment variables in `deploy/.env`
- the shared media cache mount and its read-only assumption
